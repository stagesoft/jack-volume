...
class VolumeOSC():
	def __init__(self, gui, host, port, instance, external_instance, listen_port):
...
#		print "fader_event ch=" + str(ch) + " gain=" + str(gain_abs)
	...
	def __init__(self, nchannels, host, port, instance, external_instance, local_port):
		self.lock = threading.Lock()
		self.osc = VolumeOSC(self, host, port, instance, external_instance, local_port)
...
if __name__ == "__main__":
	...
	external_instance = "touchosc"
	local_port = 7601

	try:
		opts, args = getopt.getopt(sys.argv[1:], "c:p:h:j:e:s:")
	except getopt.GetoptError as err:
		sys.stderr.write(str(err) + '\n')
		sys.stderr.write("usage: " + sys.argv[0] + " [-c <nchannels>] [-p <jv-port>] [-h <host>] [-j <jack-volume_instance>] [-e <external-name>] [-s <listening-port>]\n")
		...
		sys.stderr.write("-e  name of the external instance\n")
#		sys.stderr.write("-q  udp port of the external instance\n")
	...
	try:
		ctrl = VolumeConnector(channels, host, port, instance, external_instance, local_port)
		ctrl.run()
	except IOError:
